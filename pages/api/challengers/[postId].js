...
export default async function handler(req, res) {
  if (req.method === "GET") {
    let reqPostId = req.headers.referer;
    let splitParams = reqPostId.split("/");
    let userPostId = splitParams[splitParams.length - 1];
    let db = (await connectDB).db("onehour");
    let userPosts = await db
      .collection("userPosts")
      .find({ postId: Number(userPostId) })
      .toArray();
    ...
    try {
      ...
      let reqPostId = req.headers.referer;
      let splitParams = reqPostId.split("/");
      let userPostId = Number(splitParams[splitParams.length - 1]);
      ...
      await db.collection("userPosts").deleteOne({ postId: userPostId });
